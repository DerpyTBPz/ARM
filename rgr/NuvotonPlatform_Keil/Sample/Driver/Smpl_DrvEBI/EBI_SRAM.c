...
uint32_t u32DataArrary[4] = {0x00000000, 0xFFFFFFFF, 0x5A5A5A5A, 0xA5A5A5A5};
void SRAM_CompareFunction(uint32_t u32Param0, uint32_t u32Param1, uint32_t u32Param2, uint32_t u32Param3)
{
	...
	if (u32Param0 == 4)		// 0x0, 0x1, 0x2, 0x3, 0x4 ... Access with offset ......
	{
		...
		u32Idx = 0;
		u32ReadData = 0;
		pu8DevPtr = (uint8_t *)DRVEBI_BASE_ADDR;
		...
		if ((uint32_t)pu8DevPtr <  (DRVEBI_BASE_ADDR+u32EBIsize))
		{
			printf("One Byte compare address error 0x%08X !\n", (uint32_t)pu8DevPtr);
			while(1);
		}
		...
		u32Idx = 0;
		u32ReadData = 0;
		pu16DevPtr = (uint16_t *)DRVEBI_BASE_ADDR;
		while (u32Idx < u32EBIsize)					
		{
			if (u32WriteMode == 0)	// From Byte Write Mode
			{
		...
		if ((uint32_t)pu16DevPtr <  (DRVEBI_BASE_ADDR+u32EBIsize))
		{
			printf("Half Word compare address error 0x%08X !\n", (uint32_t)pu16DevPtr);
			while(1);
		}
		...
		u32Idx = 0;
		u32ReadData = 0;
		pu32DevPtr = (uint32_t *)DRVEBI_BASE_ADDR;
		while (u32Idx < u32EBIsize)					
		{
			if (u32WriteMode == 0)	// From Byte Write Mode
			{
		...
		if ((uint32_t)pu32DevPtr <  (DRVEBI_BASE_ADDR+u32EBIsize))
		{
			printf("One Word compare address error 0x%08X !\n", (uint32_t)pu32DevPtr);
			while(1);
		}
	}else
	{
		...
		u32Idx = 0;
		pu8DevPtr = (uint8_t *)DRVEBI_BASE_ADDR;
		u32ReadData = (uint8_t)u32DataArrary[u32Param0];
		while (u32Idx < u32EBIsize)					
		{
		...
		u32Idx = 0;
		pu16DevPtr = (uint16_t *)DRVEBI_BASE_ADDR;
		u32ReadData = (uint16_t)u32DataArrary[u32Param0];
		while (u32Idx < u32EBIsize)					
		{
		...
		u32Idx = 0;
		pu32DevPtr = (uint32_t *)DRVEBI_BASE_ADDR;
		u32ReadData = (uint32_t)u32DataArrary[u32Param0];
		while (u32Idx < u32EBIsize)					
		{
...
void SRAM_BS616LV4017(void)
{
	...
	while (u32DataIdx < 5)
	{
		...
		}else
		{
			...
			u32WriteData = 0;
			pu8DevPtr = (uint8_t *)DRVEBI_BASE_ADDR;
			printf("	0x0, 0x1, 0x2, 0x3, 0x4 ... Access with offset(0x%08X) ... ", (uint32_t)&u32Idx);
			while (u32Idx < u32EBIsize)					
			{
