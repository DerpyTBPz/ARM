...
int main (void)
{
    ...
    SYSCLK->PLLCON.OE = 0;      /* Enable PLL output */
    ...
    /* The core clock is changed, we need to update it now */
    SystemCoreClockUpdate();
    ...
    while(1)
    {
        ...
        if(g_timer0Ticks >= preTicks + 100)
        {
            ...
            estiSysFreq = (msTicks - preMsTicks) * (SystemCoreClock/100);
        ...
        SYS->GPAMFP.PWM0_AD13 = !((GPIOB->PIN >> 15) & 1);
