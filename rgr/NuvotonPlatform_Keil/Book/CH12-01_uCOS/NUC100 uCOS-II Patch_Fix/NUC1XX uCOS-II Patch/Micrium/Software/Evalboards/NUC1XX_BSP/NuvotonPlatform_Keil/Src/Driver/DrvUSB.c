...
{
    uint32_t *pu32Dest, *pu32Src;
    int32_t i;

    /* The source address, destination address and size must be 4-byte alignment */
...
int32_t DrvUSB_Open(void * pVoid)
{
	...
	{	    
		...
		CFG_EP_SETTING[i] = (CFGx_STALL_CTL | 
					((sEpDescription[i].u32EpNum&EP_INPUT)?CFGx_EPT_IN:CFGx_EPT_OUT) 
	...
	gsUsbDevice.u32FLODET = USBD->FLODET.FLODET;
...
{
	gsUsbDevice.u32EVF = _DRVUSB_GET_EVF();

	if (gsUsbDevice.u32EVF & IEF_WAKEUP)
	{
	...
	else if (gsUsbDevice.u32EVF & IEF_FLD)
	{
		gsUsbDevice.u32FLODET = USBD->FLODET.FLODET;
	...
	else if (gsUsbDevice.u32EVF & IEF_BUS)
	{
	...
	else if (gsUsbDevice.u32EVF & IEF_USB)
	{
		gsUsbDevice.u32STS = _DRVUSB_GET_STS();
...
int32_t DrvUSB_DataIn(uint32_t u32EpNum, const uint8_t * u8Buffer, uint32_t u32Size)
{
	...
	if (u8Buffer == NULL && u32Size == 0)
		...
		psDevice->abData0[u32EpNum] = !psDevice->abData0[u32EpNum];
	
	_DRVUSB_SET_EP_TOG_BIT(u32EpId, psDevice->abData0[u32EpNum]);
	_DRVUSB_TRIG_EP(u32EpId, u32Size);
...
void DrvUSB_PreDispatchFdtEvent(S_DRVUSB_DEVICE *psDevice)
{
	...
	uint32_t u32FLODET = psDevice->u32FLODET;
...
void DrvUSB_PreDispatchEPEvent(S_DRVUSB_DEVICE *psDevice)
{
	uint32_t u32EpNum;
	uint32_t u32EpId;
	uint32_t u32EVF = psDevice->u32EVF;
	uint32_t u32STS = psDevice->u32STS;
	...
	u32EVF = *((__IO uint32_t *)(&USBD->EVF)); 
    *((__IO uint32_t *)(&USBD->EVF)) = (u32EVF & 0x00FF0000UL);
	...
	for (i=16;i<16+MAX_EP_ID;i++)
	{
	    if (u32EVF & (1 << i))
	    {
    		...
    		u32EpNum = (_DRVUSB_GET_CFG(u32EpId) & CFGx_EPT);	
    		
    		u32PacketType = (  (u32STS >> (4 + u32EpId * 3))  & STS_STS);
    		...
    		else if(u32PacketType == STS_OUT1_ACK)
    		{
    			...
    			/* Assign the toggle bit of data packet */
    			psDevice->abData0[u32EpNum] = 0;
    		}
    		else if(u32PacketType == STS_ISO)
    		{
    			u32ISOPacketType = (_DRVUSB_GET_CFG(u32EpId) & CFGx_STATE);
...
void DrvUSB_CtrlSetupGetStatus(void * pVoid)
{
	...
	else if (pInfraDevice->au8Setup[0] == 0x82)
	{
		if (u32EpId < MAX_EP_ID)
		{
			...
			au8Buf[0] = (_DRVUSB_GET_CFGP(u32EpId) & CFGPx_STALL) ? 1 : 0;
		}
		else
		{
			DrvUSB_ClrCtrlReadyAndTrigStall();
