...
void PDMA_IRQHandler(void)
{
	...
	for(i32Channel=0;i32Channel<MAX_CHANNEL_NUM;i32Channel++)  
	{
		...
		if(u32SFR & (1<<i32Channel))  										 		/* Check INT Channel */	
		{
	    	...
	    	if (inpw(&PDMA0->IER + i32Channel * CHANNEL_OFFSET) & WAR_IE)	 		/* Get INT status */	
		    	u32IntStatus = inpw(&PDMA0->ISR + i32Channel * CHANNEL_OFFSET) 
									& (inpw(&PDMA0->IER + i32Channel * CHANNEL_OFFSET) | 0x0F00);
		    else
		    	u32IntStatus = inpw(&PDMA0->ISR + i32Channel * CHANNEL_OFFSET) 
									& inpw(&PDMA0->IER + i32Channel * CHANNEL_OFFSET) ;		
			...
			if(u32IntStatus & PDMATABORT_IF)			 							/* Check Target Abort INT and clear */
			{
			   if (g_pfnPDMACallback[i32Channel][0] != 0)    		
	    		...
	    		outpw(&PDMA0->ISR + i32Channel * CHANNEL_OFFSET ,PDMATABORT_IF);
			...
			else if(u32IntStatus & PDMABLKD_IF)			 							/* Transfer Done INT and clear */
			{
		    	...
		    	outpw(&PDMA0->ISR + i32Channel * CHANNEL_OFFSET ,PDMABLKD_IF);
			}
			else 										 							/* Wrap Around INT */
			{
		    	u32WraparoundStatus = inpw(&PDMA0->ISR + + i32Channel * CHANNEL_OFFSET ) & 0x0F00;
		    	...
		    	{																	/* Check Wrap Around Method and clear */
					...
					outpw(&PDMA0->ISR + i32Channel * CHANNEL_OFFSET ,u32WraparoundStatus);	   			
		    	}		
			}
		}
