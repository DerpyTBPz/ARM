...
__align(4) uint8_t g_au8SenseKey[4];
...
//struct CBW g_sCBW;
//struct CSW g_sCSW;
__align(4) struct CBW g_sCBW;
__align(4) struct CSW g_sCSW;
...
// code = 5Ah, Mode Sense
static __align(4) uint8_t ModePage_01[12] = {
	0x01, 0x0A, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00,
	0x03, 0x00, 0x00, 0x00 };

static __align(4) uint8_t ModePage_05[32] = {
...
static __align(4) uint8_t ModePage_1B[12] = {
	0x1B, 0x0A, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00 };

static __align(4) uint8_t ModePage_1C[8] = {
	0x1C, 0x06, 0x00, 0x05, 0x00, 0x00, 0x00, 0x00 };

//======================================================
const __align(4) uint8_t MassDeviceDescriptor[LEN_DEVICE] =
{
	LEN_DEVICE,		    // bLength
...
const __align(4) uint8_t MassConfigurationBlock[LEN_CONFIG+LEN_INTERFACE+LEN_ENDPOINT*2] =
{
	LEN_CONFIG,		                                        // bLength
...
const __align(4) uint8_t MassStringDescriptor0[4] =
{
	4,				// bLength
...
const __align(4) uint8_t MassStringDescriptor1[16] =
{
	16,				// bLength
	DESC_STRING,	// bDescriptorType
...
const __align(4) uint8_t InquiryID[36] = 
{
...
void my_memcpy(void *dest, void *src, int32_t size)
{
    ...
    {
        /* src of PDMA can't be Flash */
        memcpy(dest, src, size);
    }
    else if((uint32_t)src < 0x40000000)
    {
        /* src is SRAM */
        PDMA9->SAR = (uint32_t)src;
        PDMA9->DAR = (uint32_t)dest;
        PDMA9->BCR = size;
    ...
    {
        /* src is USBD */
        PDMA8->SAR = (uint32_t)src;
        PDMA8->DAR = (uint32_t)dest;
        PDMA8->BCR = size;
...
void ByteAssign(uint8_t *p, int32_t index, uint8_t value)
{
    uint32_t tmp;
    uint8_t *p8;

    if(((uint32_t)p == 0x40060100) && (index == 0) && (value != 0))
        while(1);

    p = (uint8_t *) ((uint32_t)p & 0xFFFFFFFC);
    ...
    *((uint32_t *)p) = tmp;//get_be32((UINT8 *)&tmp);


    if(((uint32_t)p == 0x40060100) && (index == 0) && 1)
    {
        uint32_t tmp32;
        tmp32 = inp32(0x40060100);
...
uint8_t ByteRead(uint8_t *p, int32_t index)
{
    uint32_t tmp;
    uint8_t *p8;
...
int32_t UsbSetup(void)
{
	...
	switch (ByteRead(g_au8UsbSetup,0) & 0x60)
	{
		case REQ_STANDARD:
			return UsbStdReq();
		case REQ_CLASS:
...
void UsbRead(void)
{
    ...
    if (g_u32Length)
    {
    	...
    	else
    	{
			...
			SpiRead(g_u32LbaAddress, u32Len, (uint32_t)STORAGE_DATA_BUF);
...
void UsbWrite(void)
{
	...
	if (g_u32Length > MAX_PACKET_SIZE)
	{
		...
		if (g_u32Address >= (STORAGE_DATA_BUF+STORAGE_BUFFER_SIZE))
		{
			...
			if (g_sCBW.bCBWLUN==F_SPI_LUN)
				SpiWrite(g_u32SpiStartAddr, STORAGE_BUFFER_SIZE, (unsigned int)STORAGE_DATA_BUF);
		    
		    g_u32Address = STORAGE_DATA_BUF;
		    g_u32SpiStartAddr += STORAGE_BUFFER_SIZE;
		}
#endif		
	}
	else
	{
		...
		if (g_sCBW.u8OPCode == UFI_WRITE_10)
		{
			...
			if (g_sCBW.bCBWLUN==F_SPI_LUN)
			{
			    len = lba * UDC_SECTOR_SIZE + g_sCBW.dCBWDataTransferLength - g_u32SpiStartAddr;
			    if(len)
			    {
				    SpiWrite(g_u32SpiStartAddr, len, (unsigned int)STORAGE_DATA_BUF);
				}
		    }
#endif
		}
			...
			case UFI_READ_10:
			{
				...
				if(g_u32Length == 0)
				{
					...
					if(g_sCBW.bCBWLUN==F_SPI_LUN)
					{
						...
						SpiRead(g_u32Address * UDC_SECTOR_SIZE, i, (unsigned int)STORAGE_DATA_BUF);
...
#else
					g_u32Address = STORAGE_DATA_BUF;
					g_u32SpiStartAddr = get_be32(&g_sCBW.au8Data[0]) * UDC_SECTOR_SIZE;
#endif
				}
...
void UsbUsb(uint32_t u32EVF)
{
	...
	else if(u32EVF & EVF_EPTF0)
	{	
	    /* Ctrl In */
		_DRVUSB_SET_EVF(EVF_EPTF0);
		UsbInAck();
	}
	else if(u32EVF & EVF_EPTF1)
	{	
	    /* Ctrl Out */
		_DRVUSB_SET_EVF(EVF_EPTF1);
		UsbOutAck();
	}
	else if(u32EVF & EVF_EPTF2)
	{	
	    /* Bulk In */
...
void udcInit(void)
{
   ...
   _DRVUSB_ENABLE_SE0();
	...
	g_u8UsbState = USB_STATE_DETACHED;
	//outphw (USBD_MXPLD1, 0x08);	// control out 
...
uint8_t Flash_Identify(uint8_t tLUN)
{
    ...
    if (tLUN==F_SPI_LUN)
    {
        g_spiTotalSectors = SPI_STORAGE_SIZE / UDC_SECTOR_SIZE;
