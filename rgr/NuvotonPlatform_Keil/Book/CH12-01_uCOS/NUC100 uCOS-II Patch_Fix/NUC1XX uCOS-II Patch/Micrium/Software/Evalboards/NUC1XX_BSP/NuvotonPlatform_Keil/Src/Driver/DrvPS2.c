...
)
{
    ...
    {
    	DRVPS2_TXFIFO(DRVPS2_TXFIFODEPTH-1);
    }
    else
    {
    	DRVPS2_TXFIFO(u32WriteBytes - 1);
    }
	
	do
	{
		u32delayno = 0;
		while (DRVPS2_ISTXEMPTY() != 1)
		{
			u32delayno++;
			if ( u32delayno >= 0x40000000 )     	    
				return E_DRVPS2_ERR_TIMEOUT;     	       
		}
		DRVPS2_TXDATA0(pu32TxBuf[i++]);
		DRVPS2_TXDATA1(pu32TxBuf[i++]);
		DRVPS2_TXDATA2(pu32TxBuf[i++]);
		DRVPS2_TXDATA3(pu32TxBuf[i++]);
