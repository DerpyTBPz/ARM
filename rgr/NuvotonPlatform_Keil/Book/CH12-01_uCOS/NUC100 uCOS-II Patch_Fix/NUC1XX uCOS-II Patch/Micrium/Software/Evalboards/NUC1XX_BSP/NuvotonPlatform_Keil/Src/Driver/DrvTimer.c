...
static int32_t  bIsTimer0Initial     = FALSE,
                ...
                bIsSetWdtEvent       = FALSE;

static int32_t volatile bIsWdtTimeOut = FALSE;
...
/*---------------------------------------------------------------------------------------------------------*/
void DrvWDT_SetEvent(TIMER_CALLBACK pvFun)
{
    bIsSetWdtEvent = TRUE;

    fnCallBack_WDT =  (TIMER_CALLBACK)pvFun;
}
...
int32_t DrvTIMER_Open(TIMER_CHANNEL channel, uint32_t uTicksPerSecond, TIMER_OPMODE mode )
{
    ...
    switch (channel)
    {
        case TMR0:
        {
			...
			outpw(&TIMER0->TCSR ,0 );                         /* disable timer                              */
			...
			uRegTicr =  GetTimerCLk(TMR0) /uTicksPerSecond;

			if(uRegTicr > 0xFFFFFF)							  /* Init count is larger than 24bit            */		
			{
				return E_DRVTIMER_CMD;			
			}

			TIMER0->TICR = uRegTicr;
		
			outpw(&TIMER0->TCSR,  (uint32_t)uRegTcr);   
        ...
        case TMR1:
        {
			...
			outpw(&TIMER1->TCSR ,0 );                       	/* disable timer                               */
            ...
            uRegTicr = GetTimerCLk(TMR1) /uTicksPerSecond;
			...
			TIMER1->TICR = uRegTicr;
			outpw(&TIMER1->TCSR, (uint32_t)uRegTcr);
        ...
        case TMR2:
        {
			...
			outpw(&TIMER2->TCSR ,0 );                              	/* disable timer                               */
            ...
            uRegTicr = GetTimerCLk(TMR2) /uTicksPerSecond;
			...
			TIMER2->TICR = uRegTicr;
			outpw(&TIMER2->TCSR, (uint32_t)uRegTcr);
		...
		case TMR3:
        {
			...
			outpw(&TIMER3->TCSR ,0 );                               /* disable timer                               */
			...
			uRegTicr = GetTimerCLk(TMR3) /uTicksPerSecond;
			...
			TIMER3->TICR = uRegTicr;
			outpw(&TIMER3->TCSR, (uint32_t)uRegTcr);
...
void WDT_IRQHandler(void)
{
    ...
    if (bIsSetWdtEvent==TRUE)                           /* Install WDT Callback Function                   */
    {
         fnCallBack_WDT(NULL);
    }
