...
static void PS2Mouse_IRQHandler(uint32_t u32IntStatus)
{
	...
	if(reg & DRVPS2_RXINT)
	{
		DRVPS2_INTCLR(DRVPS2_RXINT);
		//rxData = PS2->RXDATA;
		rxData = DRVPS2_RXDATA();
		DBG_PRINTF("rxData=%x\n", rxData);
			...
			if(g_cmd[0])
			{
				...
				if(g_cmd[0] == PS2CMD_SET_SAMPLE_RATE)
				{
        	        ...
        	        {   
        	            ...
        	            DRVPS2_SWOVERRIDE(1, 0);//inhibit communication
        	            while(1);
        	        }
...
/*----------------------------------------------------------------------------
  PS2 mouse main routine
  Parameter:
  	None
 *----------------------------------------------------------------------------*/
static void Ps2Mouse(void)
{
    Ps2Init();
    g_cmd[0] = 0;
    ...
    while(1)
    {
        if(g_opMode == PS2MOD_RESET)
        {
            ...
            while(DRVPS2_ISTXEMPTY() == 0);
            //PS2->TXDATA[0] = PS2PASS;
            DRVPS2_TXDATA(PS2PASS, 1);

            /* Report device ID */
            while(DRVPS2_ISTXEMPTY() == 0);
            DRVPS2_TXDATA(DEVICE_ID, 1);
