...
static void BaudRateCalculator(uint32_t clk, uint32_t baudRate, UART_BAUD_T *baud)
{
	...
	else							  /* Source Clock mod 16 >3 => Up 5% Error BaudRate */
	{
		...
		{
			baud->DIVX_EN = 1;		  /* Try to Set Divider X up 10 (MODE#2) */
...
/*                                                                                                         */
/* Parameter:        																					   */	
/*				 Port 	 -[in] UART Channel:  UART_PORT0 / UART_PORT1 /UART_PORT2                  		   */
...
int32_t DrvUART_Open(UART_PORT port, STR_UART_T *sParam)
{
	...
	else if(port == UART_PORT1)
	{
		...
		/* Set Parity & Data bits & Stop bits */
		UART1->LCR.SPE	=((sParam->u8cParity)&0x4)?1:0;
		UART1->LCR.EPE	=((sParam->u8cParity)&0x2)?1:0;
		UART1->LCR.PBE	=((sParam->u8cParity)&0x1)?1:0;
	...
	else 
	{
		...
		/* Set Parity & Data bits & Stop bits */
		UART2->LCR.SPE	=((sParam->u8cParity)&0x4)?1:0;
		UART2->LCR.EPE	=((sParam->u8cParity)&0x2)?1:0;
		UART2->LCR.PBE	=((sParam->u8cParity)&0x1)?1:0;
		UART2->LCR.WLS	=sParam->u8cDataBits;
		UART2->LCR.NSB	=sParam->u8cStopBits;
...
/*                                                                                                         */
/* Parameter:                                                                                              */
/*				 u16Por		   	  -[in] UART Channel:  UART_PORT0 / UART_PORT1  /  UART_PORT2              */
/*               u32InterruptFlag -[in]	DRVUART_LININT/DRVUART_WAKEUPINT/DRVUART_BUFERRINT/DRVUART_RLSNT   */
/*										DRVUART_MOSINT/DRVUART_THREINT/DRVUART_RDAINT/DRVUART_TOUTINT      */
/* Returns:                                                                                                */
/*               None                                                                                      */
...
/*                                                                                                         */
/* Parameter:        																					   */	
/*				 u16Port	   	  	  -[in]   UART Channel:  UART_PORT0 / UART_PORT1 / UART_PORT2          */
...
void DrvUART_SetFIFOTriggerLevel(UART_PORT	u16Port,uint16_t u32TriggerLevel)
{
	if(u16Port == UART_PORT0)
		...
		UART2->FCR.RFITL = u32TriggerLevel;
...
/*                                                                                                         */
/* Parameter:        																					   */	
/*				 UART_PORT	   	  	  -[in]   UART Channel:  UART_PORT0 / UART_PORT1                       */
...
void DrvUART_OpenIRCR(UART_PORT u16Port,STR_IRCR_T str_IRCR )
{
	...
	{
	   	UART1->FUNSEL.LIN_EN 	= 0;
		UART1->FUNSEL.IrDA_EN 	= 1;
	...
	{
	   	UART2->FUNSEL.LIN_EN 	= 0;
		UART2->FUNSEL.IrDA_EN 	= 1;
		UART2->IRCR.RX_EN 		= str_IRCR.u8cRXSelect ;
		UART2->IRCR.TX_EN 		= str_IRCR.u8cTXSelect ;
		UART2->IRCR.TX_INV_EN	= str_IRCR.u8cInvTX ;
		UART2->IRCR.RX_INV_EN	= str_IRCR.u8cInvRX ;			
	}
}
...
void DrvUART_OpenLIN(UART_PORT u16Port,uint16_t DIRECTION,uint16_t BCNT)
{
	...
	if(u16Port == UART_PORT0)
	{
	    ...
	    UART0->LINCON.LINBCNT = BCNT;
	}
	else if(u16Port == UART_PORT1)
	{
	   	UART1->FUNSEL.LIN_EN 	= 1;
		UART1->FUNSEL.IrDA_EN 	= 0;
		if(DIRECTION == MODE_TX )
		{
		...
		UART1->LINCON.LINBCNT = BCNT;
	}
	else if(u16Port == UART_PORT2)
	{
	   	UART2->FUNSEL.LIN_EN 	= 1;
		UART2->FUNSEL.IrDA_EN 	= 0;
		if(DIRECTION == MODE_TX )
		{
			UART2->LINCON.LINTX_EN	= 1;
			UART2->LINCON.LINRX_EN	= 0;
		}
		else
		{
			UART2->LINCON.LINTX_EN	= 0;
			UART2->LINCON.LINRX_EN	= 1;		
		}
	}
