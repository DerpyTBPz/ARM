...
;Конфігурація виводів МК для роботи з UART
	...
	STR		R2, [R0, #GPB_MFP]; Визначення режиму роботи модуля UART0
	LDR		R0, =UART0_BA
	MOVS	R2, #0
	STR		R2, [R0, #UA_FUN_SEL]; Сигнал Reset для UART0
	LDR		R0, =GCR_BA	; SYS->IPRSTC2.UART0_RST = 1;
	MOVS	R2, #0x1	; SYS->IPRSTC2.UART0_RST = 0;
	LSLS	R2, #16
	STR		R2, [R0, #IPRSTC2]
	MOVS	R2, #0x0
	STR		R2, [R0, #IPRSTC2]; Ввімкнення UART0
	LDR		R0, =CLK_BA	; SYSCLK->APBCLK.UART0_EN = 1;
	MOVS	R2, #0x1;
	LSLS	R2, #16
	STR		R2, [R0, #APBCLK] ; Сигнал скидання для буферів Tx та Rx FIFO
	LDR		R0, =UART0_BA ; Set Rx Trigger Level (FCR.RFITL)
	MOVS	R2, #0x6;
	STR		R2, [R0, #UA_FCR]; Фомат даних: к-ть розр. даних, стопових біт,
	...
	STR		R2, [R0, #UA_TOR] ;Налаштування тактового сигналу для модуля UART0
